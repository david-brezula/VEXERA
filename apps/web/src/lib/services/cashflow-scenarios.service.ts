...
export async function listScenarios(
  ...
  if (error) return []
  return (data ?? []) as unknown as CashflowScenario[]
}
...
export async function createScenario(
  ...
  const { data, error } = await (supabase.from("cashflow_scenarios" as any) as any)
    .insert({
...
export async function deleteScenario(
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const { error } = await (supabase.from("cashflow_scenarios" as any) as any)
    .delete()
    .eq("id", scenarioId)
