...
export async function recordPayment(
  ...
  const inv = invoice as unknown as { total_amount: number; paid_amount: number; status: string }

  // Insert payment record
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const { data: payment, error: paymentErr } = await (supabase.from("invoice_payments" as any) as any)
    .insert({
...
export async function checkOverpayment(
  ...
  const inv = invoice as unknown as { total_amount: number; paid_amount: number }
