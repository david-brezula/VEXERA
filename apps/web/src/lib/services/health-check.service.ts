...
export async function runHealthChecks(
  ...
  try {
    ...
    const { data: run, error: runError } = await (supabase.from("health_check_runs" as any) as any)
      .insert({
    ...
    if (allIssues.length > 0) {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      await (supabase.from("health_check_results" as any) as any).insert(rows)
    }
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    await (supabase.from("health_check_runs" as any) as any)
      .update({
        status: "completed",
...
export async function calculateRiskScore(
  ...
  try {
    ...
    const run = latestRun as unknown as {
      total_issues: number
...
export async function resolveHealthCheckIssue(
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const { error } = await (supabase.from("health_check_results" as any) as any)
    .update({
      resolved: true,
