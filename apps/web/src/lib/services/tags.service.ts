...
export async function createTag(
  ...
  const { data: tag, error } = await (supabase.from("tags" as any) as any)
    .insert({
...
export async function tagEntity(
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const { error } = await (supabase.from("entity_tags" as any) as any)
...
export async function getEntitiesByTag(
  ...
  return (data ?? []) as unknown as EntityTag[]
}
