...
async function checkPeriodNotLocked(
  ...
  const period = data as unknown as { status: string } | null
...
async function generateEntryNumber(
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const { count, error } = await (supabase.from("journal_entries" as any) as any)
...
export async function createJournalEntryAction(data: {
  ...
  try {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const { data: entry, error } = await (supabase.from("journal_entries" as any) as any)
      .insert({
        org_id: orgId,
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const { error: linesError } = await (supabase.from("ledger_entries" as any) as any)
      .insert(lineInserts)
...
export async function postJournalEntryAction(
  ...
  try {
    ...
    // Fetch journal entry
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const { data: existing, error: fetchError } = await (supabase.from("journal_entries" as any) as any)
    ...
    const entry = existing as unknown as { id: string; status: string; entry_date: string }
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const { error } = await (supabase.from("journal_entries" as any) as any)
      .update({
        status: "posted",
        posted_by: user.id,
...
export async function reverseJournalEntryAction(
  ...
  try {
    ...
    const originalLines = (linesRaw ?? []) as unknown as Array<{
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const { data: reversal, error: insertError } = await (supabase.from("journal_entries" as any) as any)
      .insert({
        org_id: orgId,
        entry_number: entryNumber,
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const { error: reversalLinesError } = await (supabase.from("ledger_entries" as any) as any)
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const { error: updateError } = await (supabase.from("journal_entries" as any) as any)
      .update({
        status: "reversed",
...
export async function batchPostJournalEntriesAction(
  ...
  try {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const { data: draftsRaw, error: fetchError } = await (supabase.from("journal_entries" as any) as any)
      .select("id, status, entry_date")
      .eq("org_id", orgId)
      .in("id", entryIds)
      .eq("status", "draft")
    ...
    const drafts = draftsRaw as unknown as Array<{ id: string; status: string; entry_date: string }>
