...
async function getDefaults(supabase: SupabaseClient, orgId: string) {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const { data: settings } = await (supabase.from("organization_ledger_settings" as any) as any)
    .select("*")
    .eq("org_id", orgId)
    .maybeSingle()
...
export async function postInvoiceToLedger(
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const { data: entry, error: entryError } = await (supabase.from("journal_entries" as any) as any)
    .insert({
      org_id: orgId,
  ...
  const journalEntryId = (entry as unknown as { id: string })?.id
