...
import { parseUblInvoiceXml, type ParsedInvoice } from "@/lib/services/xml/parse-ubl"
import { parseCiiInvoiceXml } from "@/lib/services/xml/parse-cii"
...
export async function importEInvoiceAction(xmlContent: string) {
  ...
  if (parsed.items.length > 0) {
    ...
    await (supabase as any).from("invoice_items").insert(itemRows)
  }
