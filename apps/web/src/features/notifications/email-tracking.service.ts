...
export async function createTracking(
  ...
  const { data, error } = await supabase.from("email_tracking")
    .insert({
...
export async function recordOpen(
  ...
  const { data: existing, error: fetchErr } = await supabase.from("email_tracking")
  ...
  const { error: updateErr } = await supabase.from("email_tracking")
    .update({
      status: "opened",
...
export async function getTrackingForInvoice(
  ...
  if (error) return []
  return (data ?? []) as EmailTrackingRecord[]
}
