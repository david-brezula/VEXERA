...
const FORMAT_OPTIONS: Array<{
  ...
  { value: "pohoda", label: "Pohoda XML", description: "Pre účtovný softvér Pohoda" },
  { value: "money_s3", label: "Money S3 CSV", description: "Kompatibilné s Money S3" },
  { value: "kros", label: "KROS CSV", description: "Pre účtovný systém KROS" },
  { value: "csv_generic", label: "Generic CSV", description: "Univerzálny tabuľkový formát" },
]
...
function StatusBadge({ status }: { status: ExportJob["status"] }) {
  if (status === "pending") {
    return <Badge variant="secondary">V poradí</Badge>
  }
...
function ExportFormCard({ orgId }: { orgId: string }) {
  ...
  async function onSubmit(values: ExportFormValues) {
    ...
    try {
      ...
      toast.success("Export zaradený — budete upozornení keď bude hotový")
    } catch (err) {
      toast.error(err instanceof Error ? err.message : "Nepodarilo sa zaradiť export")
    } finally {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Vygenerovať export</CardTitle>
        <CardDescription>
          Vyberte formát a obdobie na export účtovných dát.
        </CardDescription>
      </CardHeader>
      <CardContent>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>Obdobie od</FormLabel>
                    <FormControl>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>Obdobie do</FormLabel>
                    <FormControl>
                      <Input type="date" {...field} />
                    </FormControl>
                    <FormMessage />
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription>Vaše posledné exporty</CardDescription>
          </div>
          <Button
            variant="outline"
            size="sm"
      ...
      <CardContent>
        {isLoading ? (
          <p className="py-8 text-center text-sm text-muted-foreground">
            Načítavam históriu exportov...
          </p>
        ) : jobs.length === 0 ? (
          <div className="rounded-lg border border-dashed py-12 text-center">
            <p className="text-sm font-medium">Zatiaľ žiadne exporty</p>
            <p className="mt-1 text-xs text-muted-foreground">
              Vygenerujte prvý export vyššie.
            </p>
          </div>
        ) : (
...
export function ExportPageClient() {
  ...
  if (!activeOrg) {
    return (
      <p className="text-sm text-muted-foreground">Nie je vybraná organizácia.</p>
    )
  }
