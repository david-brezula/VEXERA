...
} from "@/features/reports/cashflow/actions"
import { exportCashflowReportPdfAction } from "@/features/export/actions-report"
import type { CashFlowSummary, CashFlowPoint } from "@/features/reports/cashflow/data"
import type { CashflowScenario, ScenarioAdjustment } from "@/features/reports/cashflow/scenarios.service"
import { applyScenarioAdjustments } from "@/features/reports/cashflow/scenarios.service"
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Predpoveď cash flow</h1>
          <p className="text-sm text-muted-foreground">
            90-dňová projekcia so scenárovou analýzou
          </p>
        </div>
      ...
      {summary && (
        <div className="grid gap-4 md:grid-cols-5">
          <SummaryCard
            ...
            subtitle="Celkom na bankových účtoch"
          />
          <SummaryCard
            title="Predpoveď na 30 dní"
            value={formatEur(summary.forecast_30d)}
            trend={trend30d}
            trendUp={trendUp}
          />
          <SummaryCard
            title="Predpoveď na 60 dní"
            value={formatEur(summary.forecast_60d)}
          />
          <SummaryCard
            title="Predpoveď na 90 dní"
            value={formatEur(summary.forecast_90d)}
            isAtRisk={isAtRisk}
          />
          <Card className={isAtRisk ? "border-destructive" : ""}>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-medium">Dátum rizika</CardTitle>
            </CardHeader>
            <CardContent>
              {isAtRisk ? (
                <>
                  ...
                  <div className="flex items-center gap-1 mt-1">
                    ...
                    <span className="text-xs text-destructive">Zostatok klesne do mínusu</span>
                  </div>
                </>
              ) : (
                <>
                  <div className="text-2xl font-bold text-green-600">Žiadny</div>
                  <p className="text-xs text-muted-foreground mt-1">
                    Neočakáva sa záporný zostatok
                  </p>
                </>
              )}
            </CardContent>
      ...
      <Card>
        <CardHeader className="pb-3">
          ...
          <CardDescription className="text-xs">
            Predpokladaný zostatok so scenárovými prekrytiami
          </CardDescription>
        </CardHeader>
        <CardContent>
          <CashflowAreaChart
    ...
    <Card>
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          <div>
            <CardTitle className="text-sm font-semibold">Scenáre</CardTitle>
            <CardDescription className="text-xs">
              Čo-ak scenáre na modelovanie rôznych výsledkov cash flow
            </CardDescription>
          </div>
          <Button
            variant="outline"
      ...
      <CardContent className="space-y-4">
        ...
        {showForm && (
          <div className="rounded-lg border p-4 space-y-3">
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  placeholder="napr. Nová zmluva s klientom"
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="space-y-1.5">
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem value="add_inflow">Pridať príjem</SelectItem>
                    <SelectItem value="add_outflow">Pridať výdavok</SelectItem>
                    <SelectItem value="delay_payment">Odložiť platbu</SelectItem>
                    <SelectItem value="remove_item">Odstrániť položku</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              <div className="space-y-1.5">
            ...
            <div className="flex gap-2 pt-1">
              ...
              >
                {isPending ? "Vytváram..." : "Vytvoriť scenár"}
              </Button>
              <Button
                size="sm"
                variant="ghost"
        ...
        {isLoading ? (
          ...
          <p className="text-sm text-muted-foreground py-4 text-center">
            Zatiaľ žiadne scenáre. Pridajte scenár na modelovanie rôznych výsledkov.
          </p>
        ) : (
          <div className="space-y-2">
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  <Badge variant="secondary" className="text-xs">
                    {scenario.adjustments.length} úprav
                    {scenario.adjustments.length !== 1 ? "(a)" : ""}
                  </Badge>
                  <Button
                    variant="ghost"
                    size="icon"
