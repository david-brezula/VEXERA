...
import { getActiveVatRates } from "@/shared/services/legislative.service"
...
export async function calculateVatReturn(
  ...
  // Column names use actual rate values (e.g. vat_output_23 for 23% rate)
  // Build column names dynamically from the active legislative rates
  const stdRateLabel = String(standardRate)
  const redRateLabel = String(reducedRate1)
  const payload: Record<string, unknown> = {
    organization_id: orgId,
    period_year: year,
    period_month: month,
    [`vat_output_${stdRateLabel}`]: round(vatBuckets.output_standard),
    [`vat_output_${redRateLabel}`]: round(vatBuckets.output_reduced1),
    vat_output_5: round(vatBuckets.output_5),
    [`vat_input_${stdRateLabel}`]: round(vatBuckets.input_standard),
    [`vat_input_${redRateLabel}`]: round(vatBuckets.input_reduced1),
  ...
  if (upsertErr) {
    throw new Error(`Failed to save VAT return: ${upsertErr.message}`)
  }

  return upserted as VatReturn
}
...
/** Calculate VAT return for an entire quarter (3 months aggregated). */
export async function calculateQuarterVatReturn(
  supabase: SupabaseClient,
  orgId: string,
  year: number,
  quarter: number,
  ...
  const results = await Promise.all(
    months.map((m) => calculateVatReturn(supabase, orgId, year, m))
  )

  // Aggregate all 3 months into a single return
  const aggregated = results.reduce(
    (acc, r) => ({
      vat_output_23: acc.vat_output_23 + (r.vat_output_23 ?? 0),
      vat_output_19: acc.vat_output_19 + (r.vat_output_19 ?? 0),
      vat_output_5: acc.vat_output_5 + (r.vat_output_5 ?? 0),
      vat_input_23: acc.vat_input_23 + (r.vat_input_23 ?? 0),
      vat_input_19: acc.vat_input_19 + (r.vat_input_19 ?? 0),
      vat_input_5: acc.vat_input_5 + (r.vat_input_5 ?? 0),
      total_output_vat: acc.total_output_vat + (r.total_output_vat ?? 0),
      total_input_vat: acc.total_input_vat + (r.total_input_vat ?? 0),
      vat_liability: acc.vat_liability + (r.vat_liability ?? 0),
      taxable_base_output: acc.taxable_base_output + (r.taxable_base_output ?? 0),
      taxable_base_input: acc.taxable_base_input + (r.taxable_base_input ?? 0),
      document_count: acc.document_count + (r.document_count ?? 0),
    }),
    {
      vat_output_23: 0,
      vat_output_19: 0,
      ...
      total_input_vat: 0,
      vat_liability: 0,
      taxable_base_output: 0,
      taxable_base_input: 0,
      document_count: 0,
    }
  )
  ...
  return {
    ...
    organization_id: orgId,
    period_year: year,
    period_month: startMonth, // first month of the quarter
    vat_output_23: round(aggregated.vat_output_23),
    vat_output_19: round(aggregated.vat_output_19),
    vat_output_5: round(aggregated.vat_output_5),
    vat_input_23: round(aggregated.vat_input_23),
    vat_input_19: round(aggregated.vat_input_19),
    vat_input_5: round(aggregated.vat_input_5),
    total_output_vat: round(aggregated.total_output_vat),
    total_input_vat: round(aggregated.total_input_vat),
    vat_liability: round(aggregated.vat_liability),
    taxable_base_output: round(aggregated.taxable_base_output),
    taxable_base_input: round(aggregated.taxable_base_input),
    status: "draft",
    document_count: aggregated.document_count,
    computed_at: new Date().toISOString(),
    finalized_at: null,
    finalized_by: null,
    notes: null,
