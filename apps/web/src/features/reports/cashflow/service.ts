...
export async function forecast(
  ...
  // For overdue invoices, spread across the next 7 days instead of lumping into day 0
  const todayKey = toDateKey(today)

  const inflowByDate = new Map<string, CashFlowItem[]>()
  for (const inv of inflows) {
    const dueDate = inv.due_date ?? todayKey
    const amount = Number(inv.total) || 0
    const isOverdue = dueDate < todayKey

    if (isOverdue) {
      // Spread overdue amount across next 7 days with decreasing confidence
      const spreadDays = 7
      const dailyAmount = amount / spreadDays
      for (let s = 0; s < spreadDays; s++) {
        const key = toDateKey(addDays(today, s))
        if (!inflowByDate.has(key)) inflowByDate.set(key, [])
        inflowByDate.get(key)!.push({
          description: `Overdue invoice from ${inv.client_name ?? "unknown"}`,
          amount: dailyAmount,
          direction: "inflow",
          source: "invoice",
          confidence: Math.max(0.3, 0.6 - s * 0.05),
        })
      }
    } else {
      if (!inflowByDate.has(dueDate)) inflowByDate.set(dueDate, [])
      inflowByDate.get(dueDate)!.push({
  ...
  for (const inv of outflows) {
    const dueDate = inv.due_date ?? todayKey
    const amount = Math.abs(Number(inv.total) || 0)
    const isOverdue = dueDate < todayKey

    if (isOverdue) {
      // Overdue payables are expected to be paid soon — spread across next 3 days
      const spreadDays = 3
      const dailyAmount = amount / spreadDays
      for (let s = 0; s < spreadDays; s++) {
        const key = toDateKey(addDays(today, s))
        ...
        outflowByDate.get(key)!.push({
          description: `Overdue payment to ${inv.client_name ?? "unknown"}`,
          amount: dailyAmount,
          direction: "outflow",
          source: "invoice",
          confidence: 0.9,
      ...
      if (!outflowByDate.has(dueDate)) outflowByDate.set(dueDate, [])
      outflowByDate.get(dueDate)!.push({
