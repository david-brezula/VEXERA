...
export async function getCashFlowData(orgId: string): Promise<{
  ...
  for (let d = 0; d < 90; d++) {
    ...
    // Recurring patterns due on this day — iterate from next_expected_at
    let recurringIn = 0
    let recurringOut = 0
    for (const p of patterns) {
      if (p.next_expected_at && p.frequency_days > 0) {
        // Walk occurrences forward from the last known date
        let nextOccurrence = new Date(p.next_expected_at)
        // Skip past dates before today
        while (nextOccurrence < today) {
          nextOccurrence = new Date(nextOccurrence)
          nextOccurrence.setDate(nextOccurrence.getDate() + p.frequency_days)
        }
        // Check if this occurrence lands on the current forecast date
        const occurrenceStr = nextOccurrence.toISOString().split("T")[0]
        if (occurrenceStr === dateStr) {
