...
export async function buildOrgContext(
  ...
  const orgInfo = org as { name: string; country: string; currency: string } | null
  ...
  const uniqueCategories = [...new Set(
    ((categories ?? []) as { category: string }[]).map(c => c.category)
  )]
  ...
  const arTotal = ((arData ?? []) as { total_amount: number }[])
  ...
  const apTotal = ((apData ?? []) as { total_amount: number }[])
    .reduce((sum, inv) => sum + (inv.total_amount ?? 0), 0)
  ...
  const monthExpenseTotal = ((monthExpenses ?? []) as { total_amount: number }[])
...
export async function fetchRelevantData(
  ...
  if (lowerQuery.includes("tržb") || lowerQuery.includes("revenue") || lowerQuery.includes("príj") ||
    ...
    if (docs && docs.length > 0) {
      const typedDocs = docs as { document_type: string; category: string; total_amount: number; currency: string; issue_date: string }[]
    ...
    if (invoices && invoices.length > 0) {
      const totalInvoiced = (invoices as { total_amount: number }[])
  ...
  if (lowerQuery.includes("overdue") || lowerQuery.includes("splatn") || lowerQuery.includes("nezaplat") || lowerQuery.includes("dlh")) {
    ...
    if (overdue && overdue.length > 0) {
      ...
      for (const inv of overdue as { customer_name: string; total_amount: number; due_date: string; currency: string }[]) {
  ...
  if (lowerQuery.includes("dodávateľ") || lowerQuery.includes("supplier") || lowerQuery.includes("vendor")) {
    ...
    if (suppliers && suppliers.length > 0) {
      ...
      for (const doc of suppliers as { supplier_name: string; total_amount: number }[]) {
