...
import { paginationRange, type PaginationParams, type PaginatedResult } from "@/shared/lib/pagination"
...
export async function getInvoices(
  ...
  if (filters?.search) {
    ...
    if (s) {
      query = query.or(
        `invoice_number.ilike.%${s}%,supplier_name.ilike.%${s}%,customer_name.ilike.%${s}%`
      )
    }
  }
