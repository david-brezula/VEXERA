...
import { postInvoiceToLedger } from "@/features/ledger/actions-posting"
import { getInvoiceTemplateSettingsAction } from "./actions-template"
import type { InvoiceTemplateSettings } from "./types"
import { DEFAULT_NUMBERING_FORMAT } from "./types"
import type { InvoiceNumberingFormat } from "./types"
import {
  formatInvoiceNumber,
  buildNumberLikePattern,
  parseSequenceNumber,
...
async function getNumberingFormat(
  supabase: SupabaseClient,
  orgId: string
): Promise<InvoiceNumberingFormat> {
  ...
  return {
    ...
    ...((data?.invoice_template_settings as Record<string, unknown> | null)?.numberingFormat as Record<string, unknown> ?? {}),
  }
}
...
async function generateInvoiceNumber(
  ...
  const fmt = format ?? await getNumberingFormat(supabase, orgId)
  const likePattern = buildNumberLikePattern(fmt, type)
  ...
  const lastNum = last ? parseSequenceNumber(last, fmt.separator) : 0
  return formatInvoiceNumber(fmt, type, lastNum + 1)
}

// ─── getNextInvoiceNumberAction (preview for the form) ───────────────────────
...
export async function getNextInvoiceNumberAction(
  ...
  try {
    ...
    if (!orgId) return { number: "", error: "No active organization" }

    const number = await generateInvoiceNumber(supabase, orgId, invoiceType)
    return { number }
  } catch (err) {
    return { number: "", error: err instanceof Error ? err.message : "Unexpected error" }
  }
}
...
export async function createInvoiceAction(
  ...
  try {
    ...
    if (values.contact_id) {
      await updateContactStats(supabase, values.contact_id, total)
    }
...
export async function createCreditNoteAction(
  ...
  try {
    ...
    const orig = original as Database["public"]["Tables"]["invoices"]["Row"] & { invoice_items: Database["public"]["Tables"]["invoice_items"]["Row"][] }
    ...
    if (originalItems.length > 0) {
      const creditItems = originalItems.map((item, i) => ({
