...
import { enqueueJob } from "@/shared/services/queue.service"
...
export async function createTemplate(
  ...
  const { data, error } = await supabase.from("recurring_invoice_templates")
    .insert({
...
export async function updateTemplate(
  ...
  const { error } = await supabase.from("recurring_invoice_templates")
    .update(updates)
    .eq("id", templateId)
...
export async function processRecurringInvoices(
  ...
  for (const template of templates as RecurringInvoiceTemplate[]) {
