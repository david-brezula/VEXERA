...
export async function createAccountAction(data: {
  ...
  try {
    ...
    const { data: account, error } = await supabase.from("chart_of_accounts")
      .insert({
...
export async function updateAccountAction(
  ...
  try {
    ...
    const { data: existing, error: fetchError } = await supabase.from("chart_of_accounts")
...
export async function toggleAccountActiveAction(
  ...
  try {
    ...
    if (acc.is_active) {
      const { count, error: countError } = await supabase.from("ledger_entries")
