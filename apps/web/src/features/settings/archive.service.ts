...
export async function setRetentionPolicies(
  ...
  const existingTypes = new Set(
    ((existing ?? []) as { document_type: string }[]).map((p) => p.document_type)
  )
  ...
  for (const [docType, years] of Object.entries(DEFAULT_RETENTION)) {
    ...
    const { error } = await supabase.from("archive_policies")
      .insert({
...
export async function updateRetentionPolicy(
  ...
  const { data, error } = await supabase.from("archive_policies")
    .update(input)
    .eq("id", policyId)
    .eq("organization_id", organizationId)
    .select("*")
