...
export function DocumentUploader({ invoiceId, onSuccess }: Props) {
  ...
  function validateFile(f: File): string | null {
    if (!ACCEPTED_MIME_TYPES.includes(f.type)) {
      return `Nepodporovaný typ súboru: ${f.type}. Povolené: PDF, PNG, JPG, XLSX.`
    }
    if (f.size > MAX_FILE_SIZE_BYTES) {
      return `Súbor je príliš veľký. Maximálna veľkosť je 20 MB.`
    }
    return null
  }
          ...
          {file ? (
            ...
            <>
              ...
              <p className="text-sm font-medium">
                Presuňte súbor sem alebo <span className="text-primary underline">vyberte</span>
              </p>
        ...
        {file && (
          <div className="grid grid-cols-2 gap-4">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Názov dokladu</FormLabel>
                  <FormControl>
                    <Input placeholder="Faktúra od dodávateľa" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
