...
function DocumentCard({ doc, onDeleted }: { doc: DocumentRow; onDeleted: () => void }) {
  ...
  async function handlePreview() {
    ...
    try {
      ...
      toast.error("Nepodarilo sa načítať náhľad")
    } finally {
  ...
  return (
    <>
      ...
      <Dialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Vymazať doklad?</DialogTitle>
          </DialogHeader>
          <p className="text-sm text-muted-foreground">
            <span className="font-medium text-foreground">{doc.name}</span> bude
            natrvalo odstránený. Túto akciu nie je možné vrátiť.
          </p>
          <DialogFooter>
            <DialogClose asChild>
      ...
      <div className="group relative rounded-lg border bg-card p-4 flex flex-col gap-3 backdrop-blur-xl hover:shadow-lg hover:bg-white/80 dark:hover:bg-white/8 transition-all duration-200 cursor-pointer">
        ...
        <div className="flex items-center gap-1 mt-auto">
          ...
          >
            ...
            {isImage(doc.mime_type) ? "Náhľad" : "Otvoriť"}
          </Button>
