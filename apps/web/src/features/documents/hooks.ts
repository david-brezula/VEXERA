...
import type { DocumentUploadFormValues } from "@/features/documents/schemas"
...
export function useUploadDocument() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const ALLOWED_TYPES = ["application/pdf", "image/jpeg", "image/png", "image/webp", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "application/vnd.ms-excel"]
      const MAX_SIZE = 20 * 1024 * 1024 // 20 MB

      if (file && !ALLOWED_TYPES.includes(file.type)) {
        throw new Error("Unsupported file type. Accepted: PDF, JPEG, PNG, WebP, Excel")
      }
