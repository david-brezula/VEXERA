...
import type { Notification } from "@/features/notifications/hooks"
...
function relativeTime(date: string) {
  ...
  if (minutes < 60) return `${minutes} min dozadu`
  const hours = Math.floor(minutes / 60)
  if (hours < 24) return `${hours} hod dozadu`
  return `${Math.floor(hours / 24)} d dozadu`
}
    ...
    <Popover open={open} onOpenChange={setOpen}>
      ...
      >
        ...
        <div className="flex items-center justify-between border-b px-4 py-3">
          <h3 className="text-sm font-semibold">Upozornenia</h3>
          {unreadCount > 0 && (
            <Button
              variant="ghost"
              size="sm"
